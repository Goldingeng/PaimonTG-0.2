...
async def daily(user_id):
    try:
        async with aiosqlite.connect('BD') as conn:
            cursor = await conn.execute("SELECT time, exp FROM users WHERE user_id = ?", (user_id,))
            row = await cursor.fetchone()
...
async def moon(user_id, moon):
    try:
        async with aiosqlite.connect('BD') as conn:
            cursor = await conn.execute("SELECT pass_time, exp FROM users WHERE user_id = ?", (user_id,))
            row = await cursor.fetchone()
            last_request_time = row[0]
            if moon == True:
                if datetime.datetime.now().hour != last_request_time:
...
async def twist(user_id):
    try:
        async with aiosqlite.connect('BD') as conn:
            ...
            three_star = ["Эпос о драконоборцах", "Чёрная кисть", "Холодное лезвие", "Филейный нож", "Тёмный железный меч", "Руководство по магии", "Рогатка", "Предвестник зари", "Потусторонняя история", "Посыльный", "Парный нефрит", "Меч путешественника", "Меч небесного всадника", "Меч из белого железа", "Меч драконьей крови", "Металлическая тень", "Лук ворона", "Клятва стрелка", "Изумрудный шар","Изогнутый лук", "Дубина переговоров", "Большой меч всадника", "Белая кисть", "Алебарда Миллелита"]
            for i in range(10):
                cursor = await conn.execute("SELECT exp, wallet, guarantee, hystory, hystoryEpic FROM users WHERE user_id = ?", (user_id,))
                info = await cursor.fetchone()
                    ...
                    if result == 1:
                        ...
                        if info[4] >= 10:
                            ...
                            if resultEpic == 1:
                                ...
                                reward.append(f"║ ⬜️{random.choice(three_star)}⭐⭐⭐(15💠)\n")
                                prim += 15
                ...
                async with aiosqlite.connect('BD') as conn:
                    ...
                    if wallet_row is not None:
                        ...
                        if number <= wallet:
                            if random.randint(1, 2) == 1:
                                wallet += number * 2
                                message = f"▶️Ого! Тебе повезло.🎲\nТы выиграл {wallet} примогемов! 💠"
...
async def rank(user_id):
    try:
        async with aiosqlite.connect('BD') as conn:
            cursor = await conn.execute("SELECT exp FROM users WHERE user_id = ?", (user_id,))
            ...
            if rank_new > 0:
                ...
                await conn.commit()
                message = f"""▶️Поздравляю!
Твой уровень повышен на {rank_new}!
Твоя награда: {rank_new * 1000} примогемов!
                """
            else:
                ...
                if mod == 2:
                    ...
                    if hom[0] <= 4:
                        if wallet[0] >= up_price:
                            ...
                            await conn.commit()
                            message = f"▶️Успешно!\nТы прокочал обустройство на 1 уровень!"
                        else:
                            message = f"▶️У тебя не хватает примогемов!\nСтоимость прокачки: {up_price} 💠"
                    else:
                        message = f"▶️Максимального уровня!"
                ...
                if mod == 4:
                    ...
                    if hom[0] <= 4:
                        if wallet[0] >= up_price:
                            ...
                            await conn.commit()
                            message = f"▶️Успешно!\nТы прокочал ограждение на 1 уровень!"
                        else:
                            message = f"▶️У тебя не хватает примогемов!\nСтоимость прокачки: {up_price} 💠"
                    else:
                        message = f"▶️Максимального уровня!"
        ...
        async with aiosqlite.connect('BD') as conn:
            async with conn.cursor() as cursor:
                ...
                if time[0][0] != data:
                    ...
                    message = "▶️Ты уже получал сегодня благословеие!\nПриходи завтра!"
            await cursor.close()
            return message
    except Exception as e:
        traceback.print_exc()
