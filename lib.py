...
async def twist(user_id):
    try:
        async with aiosqlite.connect('BD') as conn:
            ...
            three_star = ["Эпос драконоборцах", "Чёрная кисть", "Холодное лезвие", 
                          "Филейный нож", "Тёмный меч", "Рогатка", "Предвестник зари", 
                          "Потусторонняя история", "Посыльный", "Парный нефрит", "Меч", 
                          "Меч всадника", "Меч из железа", "Меч крови", "Металлическая тень", 
                          "Лук ворона", "Клятва стрелка", "Изумрудный шар","Изогнутый лук", 
                          "Дубина переговоров", "Большой меч", "Белая кисть", "Алебарда Миллелита"]
